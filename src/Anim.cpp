...
void Anim_DrawHand(HandAnim* anim, FreeCam* cam) {
    (void)cam; // not needed — arm is in fixed eye space

    GX_SetFog(GX_FOG_NONE, 0, 0, 0, 0, (GXColor){0,0,0,0});
    GX_SetZMode(GX_FALSE, GX_ALWAYS, GX_FALSE); // skip depth entirely
    GX_SetTevOp(GX_TEVSTAGE0, GX_PASSCLR);
    GX_SetTevOrder(GX_TEVSTAGE0, GX_TEXCOORDNULL, GX_TEXMAP_NULL, GX_COLOR0A0);
    GX_SetCullMode(GX_CULL_BACK); // restore proper culling — CULL_NONE caused the ghost box

    // ── Eye-space position (no view matrix needed) ────────────────────────
    // In eye space: X right, Y up, -Z forward.
    // Place pivot so arm comes from bottom-right corner, extending down off-screen.
    float ex =  0.38f + anim->arm.pos.x * 0.5f;  // far right
    float ey = -0.36f + anim->arm.pos.y;          // well below centre
    float ez = -0.50f;                             // in front of near plane

    // Rest rotations: tilt arm inward (-25 Y) and slightly down (-10 X)
    float restRY = -25.0f;
    float restRX = -10.0f + anim->arm.rot.x;
    float restRZ =  anim->arm.rot.z;
    ...
    // Shift arm downward inside its own local space so top sits at pivot
    Mtx shift, final;
    guMtxIdentity(shift);
    ...
    draw_box(0.20f, 0.50f, 0.20f, 198, 145, 100); // bigger arm
